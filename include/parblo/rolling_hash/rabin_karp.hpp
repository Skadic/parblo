...
class RabinKarp {
    ...
    /// \brief When we need to remove a char from the hash we would actually need to multiply it by `BASE`^`window_size`
    /// and then subtract it. However since our hash is in the finite field GF(PRIME), multiplying with the remainder is
    /// equivalent.
  ...
  public:
    /// \brief Create the remainder of BASE^(window_size) modulo PRIME
    /// \param window_size The window size of the hasher.
    /// \return The remainder of `BASE`^`window_size` modulo `PRIME`
    ...
    /// \brief Create a new Rabin-Karp hasher from a pointer, with a given string length and window size with a
    ///     pre-calculated remainder.
